...
struct CustomUiMaterial {
    ...
    @location(6) time: f32,
    @location(7) resolution: vec3<f32>,
...
fn fragment(in: UiVertexOutput) -> @location(0) vec4<f32> {
    ...
    var color = select(
        turbulence(in, input.background_color),
...
fn turbulence(in: UiVertexOutput, base_color: vec4<f32>) -> vec4<f32> {
    ...
    var uv = in.uv / input.size.yx + in.position.xy / input.resolution.xy;
    ...
    //color = clamp(color + vec3(0.0, 0.35, 0.5), vec3(0.0), vec3(1.0));
    color = color * input.power;
    color = clamp(color + base_color.rgb, vec3(0.0), vec3(1.0));
